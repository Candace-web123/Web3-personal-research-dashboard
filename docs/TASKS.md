...
## 2.7 TASK-025：首页决策流程 / UI 产品结构优化（已完成）

**阶段：** Phase 8｜**优先级：** P1｜**状态：** 已完成（已合并至 `main`）
...
- [x] 与墨刀 `web3-research-v12_v4.html` 视觉细节完全对齐（可迭代）
...
25. TASK-025（首页决策流程 / UI 产品结构优化，**已完成**）
...
**代码基线：** TASK-001～025 全部完成（V1.2 MVP）。V1.3 AI 引擎 + 实时数据（P0–P3）已完成；P4-0 动态渲染（`force-dynamic`）已上线。

**V1.3 已完成（P0–P4-0）：**

| 阶段 | 内容 | 状态 |
|------|------|------|
| P0 | 市场状态机引擎（7 状态 × 9 维度） | 已完成 |
| P1 | 风险模式选择器 + 仓位计算器 | 已完成 |
| P2 | AI 决策编排器 + 审计追踪 | 已完成 |
| P3 | 实时数据接入（CoinGecko / Alternative.me / DeFiLlama / CoinGlass）+ 用户持仓 | 已完成 |
| P4-0 | `force-dynamic` 动态渲染（每请求拉取实时数据，不再 build-time 静态生成） | 已完成 |

**下一步：**

1. P4 回测 / 历史复盘（backtesting + historical review）
2. 或进入 PRD 第二十六章「未来数据接入」相关迭代
3. 补充手动覆盖 UI（PRD 21.7，可单开 TASK）

### 下一轮可直接复制给 Cursor 的执行 Prompt（P4 回测）

```text
请基于当前 lib/ 引擎（market-state-engine.ts / risk-mode-selector.ts / portfolio.ts）设计回测框架。

要求：
1. 输入历史 BTC 价格 + 链上指标 + 宏观数据 → 回放引擎判断
2. 输出每日状态判定 vs 实际走势对比
3. 统计准确率、误判类型、改进建议
4. 不修改现有引擎逻辑，仅新增 lib/backtest.ts
```
...
以下内容来自 PRD 守边界版 **明确暂缓** 或 V1.3 之后的迭代：
...
- 实时分钟级行情推送（WebSocket/SSE）—— 当前 V1.3 已实现 **per-request 实时 API 拉取**（`force-dynamic`），但非流式推送。
- 登录与多用户、付费数据源。
